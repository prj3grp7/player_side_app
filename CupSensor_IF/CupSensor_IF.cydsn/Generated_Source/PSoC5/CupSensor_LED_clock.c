/*******************************************************************************
* File Name: CupSensor_LED_clock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: CupSensor_LED_clock_Start
********************************************************************************
*
* Summary:
...
void CupSensor_LED_clock_Start(void) 
{
    ...
    CupSensor_LED_clock_CLKEN |= CupSensor_LED_clock_CLKEN_MASK;
	CupSensor_LED_clock_CLKSTBY |= CupSensor_LED_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CupSensor_LED_clock_Stop
********************************************************************************
*
* Summary:
...
void CupSensor_LED_clock_Stop(void) 
{
    ...
    CupSensor_LED_clock_CLKEN &= (uint8)(~CupSensor_LED_clock_CLKEN_MASK);
	CupSensor_LED_clock_CLKSTBY &= (uint8)(~CupSensor_LED_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CupSensor_LED_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CupSensor_LED_clock_CLKEN & CupSensor_LED_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CupSensor_LED_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CupSensor_LED_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CupSensor_LED_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CupSensor_LED_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CupSensor_LED_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CupSensor_LED_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CupSensor_LED_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CupSensor_LED_clock_GetSourceRegister();
    uint16 oldDivider = CupSensor_LED_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CupSensor_LED_clock_CLKEN & CupSensor_LED_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CupSensor_LED_clock_DIV_PTR, clkDivider);
                CupSensor_LED_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CupSensor_LED_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CupSensor_LED_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CupSensor_LED_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(CupSensor_LED_clock_DIV_PTR, clkDivider);
				CupSensor_LED_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: CupSensor_LED_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CupSensor_LED_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: CupSensor_LED_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CupSensor_LED_clock_MOD_SRC |= modeBitMask & (uint8)CupSensor_LED_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: CupSensor_LED_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CupSensor_LED_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CupSensor_LED_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: CupSensor_LED_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CupSensor_LED_clock_MOD_SRC & (uint8)(CupSensor_LED_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: CupSensor_LED_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CupSensor_LED_clock_GetDividerRegister();
    uint8 oldSrc = CupSensor_LED_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CupSensor_LED_clock_MOD_SRC |= CYCLK_SSS;
        CupSensor_LED_clock_MOD_SRC =
            (CupSensor_LED_clock_MOD_SRC & (uint8)(~CupSensor_LED_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CupSensor_LED_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CupSensor_LED_clock_MOD_SRC & CupSensor_LED_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CupSensor_LED_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CupSensor_LED_clock_PHASE = clkPhase & CupSensor_LED_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CupSensor_LED_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CupSensor_LED_clock_PHASE & CupSensor_LED_clock_PHASE_MASK;
}

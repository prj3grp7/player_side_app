...
struct CupSensorUnit sensors[6] = 
{
    ...
    .hit_trigger = 1.65f, //(hit trigger value)/(placed_baseline).
...
CY_ISR(newValueInterruptHandler)
{
    ...
    if(V_sample[0] >= 0)
    {
        ...
        uint8_t oldStatus = CupSensor_cupStatus;
        if(CupSensorUnit_insertReading(&(sensors[regValue]), sampleValue))
        {
            ...
            switch(sensors[regValue].state){
                case PLACED:
                {
                    ...
                    updateCupStatus(CupSensor_cupStatus);
                }   
                break;
                case NOT_PLACED:
                {
...
//                    (*CupSensor_status_callback)(CupSensor_cupStatus);
            ...
            //if(!(CupSensor_cupStatus & 1<<1) && (oldStatus & 1<<1)) test_LED_Write(0);
        }
    }

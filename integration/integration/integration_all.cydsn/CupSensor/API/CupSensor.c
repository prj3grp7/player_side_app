...
struct CupSensorUnit sensors[6] = 
{
    ...
    .hit_trigger = 1.75f, //(hit trigger value)/(placed_baseline).
...
CY_ISR(newValueInterruptHandler)
{
    ...
    if(V_sample[0] >= 0)
    {
        ...
        CUP_STATE oldState = sensors[regValue].state;
        uint8_t oldStatus = `$INSTANCE_NAME`_cupStatus;
        if(CupSensorUnit_insertReading(&(sensors[regValue]), sampleValue))
        {
            ...
            switch(sensors[regValue].state){
                case PLACED:
                {
                    ...
                    updateCupStatus(`$INSTANCE_NAME`_cupStatus);
                }   
                break;
                case NOT_PLACED:
                {
...
//                    (*`$INSTANCE_NAME`_status_callback)(`$INSTANCE_NAME`_cupStatus);
                    if(regValue == 1) {
                        //test_LED_Write(0);
                        sprintf(uartSensorBuffer,"\n\rerror:%i\n\r",sampleValue);
                        UART_PutString(uartSensorBuffer);    
                    }
            ...
            //if(!(`$INSTANCE_NAME`_cupStatus & 1<<1) && (oldStatus & 1<<1)) test_LED_Write(0);
        }
    }

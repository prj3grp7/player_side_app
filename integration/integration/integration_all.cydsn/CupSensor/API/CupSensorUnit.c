    ...
    if(error == 0){
        if ((sensorUnit->baseline > 0))
        {
            switch (sensorUnit->state)
            {
                case NOT_PLACED:
                {
                    if((lastReading - sensorUnit->baseline) > sensorUnit->placed_trigger && (lastReading > sensorUnit->baseline))
                    {
                ...
                {
                    if((lastReading - sensorUnit->baseline) < sensorUnit->removed_trigger || (lastReading < sensorUnit->baseline))
                    {
                        sensorUnit->state = NOT_PLACED;
                        CircularBuffer_resetBuffer(&(sensorUnit->averageBuffer));
                    }
                ...
                case PLACED:
                {
                    if(((lastReading - sensorUnit->baseline) > (sensorUnit->hit_trigger * sensorUnit->placed_baseline)) && (sensorUnit->placed_baseline > 0) && (lastReading > sensorUnit->baseline))
                    {

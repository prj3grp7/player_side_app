...
I2C_slave_BACKUP_STRUCT I2C_slave_backup =
{
    ...
    I2C_slave_DEFAULT_CLKDIV1,
    I2C_slave_DEFAULT_CLKDIV2
};
...
*
* Global variables:
*  I2C_slave_backup - The non-retention registers are saved to.
*
* Reentrant:
*  No.
*
...
void I2C_slave_SaveConfig(void) 
{
    ...
    I2C_slave_backup.adr     = I2C_slave_ADDR_REG;
    ...
    I2C_slave_PWRSYS_CR1_REG |= I2C_slave_PWRSYS_CR1_I2C_BACKUP;
    ...
    */
    ...
    while (0u == (I2C_slave_XCFG_REG & I2C_slave_XCFG_SLEEP_READY))
    {
    }

     /* Setup wakeup interrupt */
    I2C_slave_DisableInt();
    I2C_slave_wakeupSource = 0u; /* Clear wakeup event */
...
*
* Global variables:
*  I2C_slave_backup - the non-retention registers are restored from.
*
* Reentrant:
*  No.
*
...
*  I2C_slave_Sleep() may produce unexpected behavior.
*
*******************************************************************************/
void I2C_slave_RestoreConfig(void) 
{
    ...
    if (0u != (I2C_slave_PWRSYS_CR1_I2C_BACKUP & I2C_slave_PWRSYS_CR1_REG))
    /* Exit from Sleep */
    {
        /* Disable I2C backup regulator */
        intState = CyEnterCriticalSection();
        I2C_slave_PWRSYS_CR1_REG &= (uint8) ~I2C_slave_PWRSYS_CR1_I2C_BACKUP;
    ...
    else /* Exit from Hibernate (bit is cleared) or wakeup disabled */
    {
        ...
        intState = CyEnterCriticalSection();
        I2C_slave_PM_ACT_CFG_REG  &= (uint8) ~I2C_slave_ACT_PWR_EN;
        I2C_slave_PM_STBY_CFG_REG &= (uint8) ~I2C_slave_STBY_PWR_EN;
        CyExitCriticalSection(intState);

        /* Enable component after restore complete */
        I2C_slave_backup.enableState = I2C_slave_ENABLED;
    #endif /* (I2C_slave_WAKEUP_ENABLED) */
        ...
        I2C_slave_ADDR_REG    = I2C_slave_backup.adr;
    ...
    if (0u != I2C_slave_wakeupSource)
    {
        ...
        (void) CyIntSetPending(I2C_slave_ISR_NUMBER);
    }
    I2C_slave_EnableInt();
#endif /* (I2C_slave_WAKEUP_ENABLED) */
}
...
*
* Global variables:
*  I2C_slave_backup - The non retention registers are saved to. Changed
*  by I2C_slave_SaveConfig() function call.
*
* Reentrant:
*  No.
*
...
{
#if (I2C_slave_WAKEUP_ENABLED)
    /* Do not re-enable component in I2C_slave_Wakeup(). It remains enabled as it is wakeup source */
    I2C_slave_backup.enableState = I2C_slave_DISABLED;
#else
    /* Store component state enabled or disable to restore after wakeup */
    I2C_slave_backup.enableState = (I2C_slave_PM_ACT_CFG_REG & I2C_slave_ACT_PWR_EN);
